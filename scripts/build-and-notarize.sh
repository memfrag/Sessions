...
# Versioning lives in the pbxproj only (GENERATE_INFOPLIST_FILE derives
# CFBundleShortVersionString/CFBundleVersion from MARKETING_VERSION and
# CURRENT_PROJECT_VERSION); the partial Info.plist must not be stamped.
...
if [ -z "$CURRENT_VERSION" ]; then
    error "Could not read MARKETING_VERSION from $PBXPROJ"
fi
